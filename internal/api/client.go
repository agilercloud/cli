// Package api implements the HTTP client for the Agiler API.
package api
...
func (c *Client) DoRaw(ctx context.Context, method, path, contentType string, headers map[string]string, body io.Reader) (*http.Response, error) {
	url := c.BaseURL + path
...
// DoJSONIdempotent executes a request with an idempotency key.
func (c *Client) DoJSONIdempotent(ctx context.Context, method, path string, body io.Reader, dest any) error {
	url := c.BaseURL + path
